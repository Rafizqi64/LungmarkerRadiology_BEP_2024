...
class VotingModel:
    ...
    def determine_custom_threshold(self, y_test, y_proba, desired_percentage):
        ...
        it calculates the NPV for each threshold and selects the one with the best performance.
        ...
        Returns:
        - float: The threshold that meets the desired performance metric percentage, defaulting to 0.5 for PPV if no
        ...
        if self.threshold_metric == 'ppv':
            ...
            return thresholds_pr[threshold_indices[0] - 1] if threshold_indices.size > 0 else 0.5
